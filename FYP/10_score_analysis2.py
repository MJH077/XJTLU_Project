...
# 关键点：只保留xx地区
df_full = df.copy()
x_min_full = df_full["video_score"].min()
x_max_full = df_full["video_score"].max()
y_min_full = df_full["comment_score"].min()
y_max_full = df_full["comment_score"].max()
df = df[df["region_en"].isin(["Shanghai", "Jiangsu", "Taiwan", "Yunnan", "Ningxia"])].copy()
df["size_score"] = np.exp(df["user_engagement_avg_score"] / 5)
video_median = df_full["video_score"].median()
comment_median = df_full["comment_score"].median()
...
y_min, y_max = y_min_full, y_max_full
fig.add_annotation(
...
# ========== 导出 ==========
html_path = r"E:\XJTLU\Year4\BUS303\FYP research\FYP\Question2_Collection\Question2_score_analysis2.html"
